...
export default function ScrollytellingSection() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Calculate progress 0 to 1 based on how far we've scrolled inside containerRef
  ...
  const handleJumpToStep = (step: number) => {
    ...
    const targetOffset = ((step - 1) / 3.2) * totalScrollDistance;
  ...
  return (
    <div ref={containerRef} className="relative min-h-[360vh] px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      {/* ── Sticky Pinned Viewport Stage (Locks on Screen) ────────── */}
      <div className="sticky top-20 z-20 py-8 space-y-8">
        {/* Section Title & Progress Ticker */}
        <div className="text-center space-y-3 max-w-3xl mx-auto">
          ...
          <div className="max-w-md mx-auto pt-2 space-y-2">
            <div className="flex items-center justify-between text-xs font-mono">
              <span className="text-purple-300 font-bold">Stage 0{activeStep} of 04: {current.subtitle}</span>
              <span className="text-zinc-500">Scroll to Advance ({scrollProgressPct}%)</span>
            </div>

            {/* Segmented Progress Track */}
            <div className="flex items-center gap-1.5 h-1.5 w-full bg-zinc-800/80 rounded-full overflow-hidden p-0.5">
              {[1, 2, 3, 4].map((s) => (
                <button
                  key={s}
                  ...
                  className={`h-full flex-1 rounded-full transition-all cursor-pointer ${
                    activeStep >= s
                      ? 'bg-gradient-to-r from-purple-500 to-cyan-400 shadow-[0_0_8px_rgba(168,85,247,0.8)]'
        ...
        {/* ── Dynamic Single-Card Split Viewport (Left Card + Right Visualizer) ── */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-5 relative">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="p-8 rounded-3xl border border-purple-500/40 bg-[#0e111d]/95 backdrop-blur-2xl shadow-[0_0_50px_rgba(147,51,234,0.2)] space-y-5"
              >
          ...
          {/* Right Column: Dynamic Visualizer Canvas (7 cols) ────────── */}
          <div className="lg:col-span-7">
            <div className="card-elevation rounded-3xl border border-white/[0.1] bg-[#07090e]/95 backdrop-blur-2xl shadow-[0_0_60px_rgba(0,0,0,0.9)] overflow-hidden h-[460px] flex flex-col justify-between">
              {/* Header */}
              <div className="p-4 bg-[#0d0f17] border-b border-white/[0.08] flex items-center justify-between">
                    ...
                    >
                      <div className="p-4 rounded-2xl bg-[#0e111a] border border-purple-500/30 space-y-3 shadow-inner">
                        ...
                        <div className="p-3 rounded-xl bg-[#06070b] border border-white/[0.06] text-xs font-mono space-y-2">
                          ...
                          <div className="pt-1 flex items-center justify-center text-purple-400">
              ...
              {/* Bottom State Bar */}
              <div className="p-3.5 bg-[#090b12] border-t border-white/[0.08] flex items-center justify-between text-xs font-mono text-zinc-400">
                <span className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-purple-400 animate-pulse" />
                  <span>State: Active in Soroban VM</span>
                </span>

                <span className="text-zinc-500 text-[11px]">Scroll down to see next stage</span>
              </div>
            </div>
          </div>
        </div>
