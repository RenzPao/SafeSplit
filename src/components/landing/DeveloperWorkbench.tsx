...
console.log('Escrow funded atomically on Stellar Soroban:', txHash);`,
...
impl SafeSplitContract {
    pub fn approve_milestone(
        ...
        assert_eq!(caller, config.client, "Only client can approve milestone");

        // Disburse milestone funds atomically to freelancer
  ...
  const faqs = [
    {
      ...
      a: 'Funds are deposited into an autonomous Stellar Soroban smart contract instance. The smart contract bytecode enforces that funds can only be released upon client signature, refunded if mutually agreed, or split by the designated neutral Arbiter.',
    },
    {
  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-16">
      ...
      <div className="space-y-8">
        ...
        {/* Code Console */}
        <div className="max-w-4xl mx-auto card-elevation rounded-2xl border border-white/[0.1] bg-[#07090e] shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden">
          {/* Header */}
          <div className="px-5 py-3.5 bg-[#0d0f17] border-b border-white/[0.08] flex items-center justify-between flex-wrap gap-3">
            ...
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] text-zinc-300 text-xs font-mono transition-colors border border-white/[0.06]"
            >
          ...
          {/* Code Viewer */}
          <div className="p-6 font-mono text-xs text-purple-200/90 overflow-x-auto min-h-[220px]">
            <pre className="whitespace-pre-wrap leading-relaxed">{codeSnippets[activeTab]}</pre>
          </div>
        </div>
      </div>

      {/* ── 2. Searchable FAQ Accordion ────────────────────────────── */}
      <div className="max-w-4xl mx-auto space-y-6 pt-10 border-t border-white/[0.06]">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-1">Everything you need to know about SafeSplit security and execution.</p>
          </div>
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full bg-[#0d0f17] border border-white/[0.08] rounded-xl pl-9 pr-3.5 py-2 text-xs text-white focus:outline-none focus:border-purple-500 placeholder:text-zinc-500 shadow-inner"
            />
          </div>
        </div>
        ...
        <div className="space-y-3">
          {filteredFaqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className="rounded-2xl border border-white/[0.06] bg-[#0b0d14]/80 overflow-hidden transition-all"
              >
                <button
                  onClick={() => setOpenFaq(isOpen ? null : idx)}
                  className="w-full p-4 text-left flex items-center justify-between gap-4 font-semibold text-xs sm:text-sm text-zinc-200 hover:text-white transition-colors"
                >
                  <span>{faq.q}</span>
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div
                      ...
                      className="px-4 pb-4 text-xs text-zinc-400 leading-relaxed border-t border-white/[0.04] pt-3 font-sans"
                    >
                      {faq.a}
                    </motion.div>
                  )}
